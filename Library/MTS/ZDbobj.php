 ...
 * @LastEditTime: 2021-01-09 18:09:06
 * @Description: 数据库操作 基础类
 * 使用方法 ：子类先实现并调用 addParams 方法 ，再调用 setQuery 方法
 */

abstract class MTS_ZDbobj
{
    protected $orm;
     ...
     *要求比较严格
     * @return void
     * @desc 当需要查找连表的字段时候就会执行 left join，没有查询左连表的字段，不会执行left join
     * 主表主键必须是id，连表主键必须是id
     * 主表必须设置外键字段，外键字段要求 表名_id
     * @example
     * @author Brightness
     * @since
     */
    ...
    {
        return $this->orm->select('monitor_offer.id,lleft.lefttext AS TEXT,monitor_image.image AS IMG')->fetchAll();
    }
    ...
    public function getByParams($fields = '*')
    {
        ...
        return  $this->orm->select($needFields)->fetchOne();
    }
     ...
     * 执行手写sql
     *
     * @param string $sql  'SELECT name FROM tbl_user WHERE age > ? LIMIT 1'
     * @param string $param array(18)
     * @return array
     * @desc 
     * @example
     * @author Brightness
    ...
    public function queryAll($sql, $params = array())
    {
        ...
        return $this->orm->queryRows($sql, $params);
    }

    /**
     * 添加筛选参数
     *
     * @param array $parmas ['id'=>1]
     * @return void
     * @desc
     * @example
     * @author Brightness
