...
class Tool_Validate
{
  ...
  {
    //参数不能是空数组
    if (empty($data_arr) or !is_array($data_arr)) return '数据不能为空';
    if (empty($rules) or !is_array($rules)) return '规则不能为空';
    if (!is_array($err_msg)) {
      $err_msg = [];
    }
    //遍历规则，检查数据
    foreach ($rules as $rule_key => $rule) {
      $checkAction_arr = $this->explodeRules($rule); #分割验证规则
      if (!empty($checkAction_arr)) {
        //遍历每个要验证的规则
        foreach ($checkAction_arr as $checkAction) {

          $checkAction = strtolower($checkAction); #校验规则

          //判断规则是否存在
          if (!in_array($checkAction, $this->rules)) {
            return 'rule name "' . $checkAction . '" is not found!'; #不存在的规则，提示没有该规则
          }

          $result = $this->$checkAction($data_arr[$rule_key]); #执行验证
          if (!$result) {
            //不符合规则，返回错误信息，没有设置错误信息，使用默认错误信息
            return $err_msg[$rule_key][$checkAction] ? $err_msg[$rule_key][$checkAction] : $rule_key . $this->errMsg[$checkAction];
          }
        }
      }
    }
